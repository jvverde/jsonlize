  ...
  function compose (obj) {
    ...
    if (obj && typeof obj === 'object' && '_ref' in obj) {
      ...
      if (obj._class === 'Set') {
        ...
        const redirect = (obj, newobj, oldobj) => { // Redirect references from oldobj to newobj
          ...
          names.forEach(name => {
            if (obj[name] === oldobj) {
              ...
              redirect(obj[name], newobj, oldobj)
            }
          })
        }
        try {
          const base = new builtins[obj._parent].type(parentValue)
