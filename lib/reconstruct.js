...
const getAllPropertyNames = require('./getAllPropertyNames')

const redirect = (obj, newref, oldref) => { // Redirect references from oldref to newref
  const map = new Map([[oldref, newref]]) // map oldref to newref
  getAllPropertyNames(oldref) // And map also all properties (include ones in the prototypes)
    .filter(name => !map.has(oldref[name])) // Only if a precious map doesn't exists yet
    .filter(name => name in newref) // And if newref[name] exists
    .forEach(name => map.set(oldref[name], newref[name]))
