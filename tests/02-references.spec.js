...
describe('Test circular an cross references', () => {
  describe('Test cloned (deserialized of serialized) objects are exactly the same', () => {
    ...
    ;['array', 'set', 'map'].forEach(v => {
      test(`The cloned {obj, array, set, map} clone.obj.${v} should be clone.${v}`, () => {
        const clone = serdes(tests)
        expect(clone.obj[v] === clone[v]).toBe(true)
      })
    })
    ;['obj', 'array', 'set', 'map'].forEach((v, k) => {
      test(`The cloned {obj, array, set, map} clone.array[${k}] should be clone.${v}`, () => {
        const clone = serdes(tests)
        expect(clone.array[k] === clone[v]).toBe(true)
      })
    })
    ;['obj', 'array', 'set', 'map'].forEach((v) => {
      test(`The cloned {obj, array, set, map} clone.set should have clone.${v}`, () => {
        const clone = serdes(tests)
        expect(clone.set.has(clone[v])).toBe(true)
      })
    })
    ;['obj', 'array', 'set', 'map'].forEach((v) => {
      test(`The cloned {obj, array, set, map} clone.map should have clone.${v}`, () => {
        const clone = serdes(tests)
        expect(clone.map.has(clone[v])).toBe(true)
      })
    })
    test(`The cloned {obj, array, set, map} clone.map.get(clone.obj) should be clone.array`, () => {
      const clone = serdes(tests)
      expect(clone.map.get(clone.obj) === clone.array).toBe(true)
    })
    test(`The cloned {obj, array, set, map} clone.map.get(clone.array) should be clone.set`, () => {
      const clone = serdes(tests)
      expect(clone.map.get(clone.array) === clone.set).toBe(true)
    })
    test(`The cloned {obj, array, set, map} clone.map.get(clone.set) should be clone.map`, () => {
      const clone = serdes(tests)
      expect(clone.map.get(clone.set) === clone.map).toBe(true)
    })
    test(`The cloned {obj, array, set, map} clone.map.get(clone.map) should be clone.obj`, () => {
      const clone = serdes(tests)
